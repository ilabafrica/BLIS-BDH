	...
	<table class="table table-bordered table-advanced table-condensed">
			...
			<?php 
				...
				foreach ($obsv as  $Observation) { ?>
					...
					<td><textarea id="txtObsv_<?php echo $test->testId ?>" style="width:390px" class="abbreviation"></textarea></td>
				...
				<?php
					...
					<td><?php echo time_elapsed_pretty() ?></td>
					<td><?php echo $_SESSION['username'] ?></td>
					<td><textarea id="txtObsv_<?php echo $test->testId ?>" class="abbreviation" style="width:390px" class="abbreviation"></textarea></td>
					<td><a class="btn mini" href="javascript:void(0)" onclick="saveObservation(<?php echo $test->testId ?>, <?php echo "'".$_SESSION['username']."'" ?>)">Save</a> </td>
					</tr>
					<?php
				}
...
<script type='text/javascript'>
	...
	function update_remarks1()
	{
		...
		update_remarks(<?php echo $test_type->testTypeId; ?>, <?php echo count($measure_list); ?>, <?php echo $patient->getAgeNumber(); ?>, '<?php  echo $patient->sex;?>', '<?php echo $test->testId?>');
	}
